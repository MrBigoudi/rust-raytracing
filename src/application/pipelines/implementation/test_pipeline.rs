use ash::vk::{DescriptorImageInfo, DescriptorSetLayoutCreateFlags, DescriptorType, ImageLayout, Pipeline, PipelineBindPoint, PipelineLayout, ShaderStageFlags, WriteDescriptorSet};
use log::error;

use crate::application::{core::error::ErrorCode, pipelines::{compute_pipeline::{ComputePipeline, PipelineAttributes}, descriptor::Descriptor}, scene::scene::Scene, vulkan::{descriptors_helper::{allocator::DescriptorPoolSizeRatio, layout_builder::DescriptorLayoutBuilder}, types::VulkanContext}};
...
impl TestPipeline{
    pub fn init_pool_size_ratios(&mut self, vulkan_context: &VulkanContext) -> Result<(), ErrorCode> {
        ...
        self.base.descriptor_allocator.init_pool(device, allocation_callback, 10, &pool_size_ratios)?;
        Ok(())
    }

    pub fn init_set_0(&mut self, vulkan_context: &VulkanContext, _scene: &Scene) -> Result<Descriptor, ErrorCode> {
        ...
        let descriptor_set_layout = layout_builder.build(
            device, allocation_callback, 
            ShaderStageFlags::COMPUTE, DescriptorSetLayoutCreateFlags::empty()
        )?;

        // Allocate the set
        let descriptor_set = self.base.descriptor_allocator.allocate(device, descriptor_set_layout)?;
...
impl ComputePipeline for TestPipeline {
    ...
    fn init_descriptors(&mut self, vulkan_context: &VulkanContext, scene: &Scene) -> Result<(), ErrorCode> {
        ...
        let set_0 = match self.init_set_0(vulkan_context, scene){
            Ok(set) => set,
            Err(err) => {
                error!("Failed to initialize the descriptor set 0 in the test pipeline: {:?}", err);
