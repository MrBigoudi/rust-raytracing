...
use ash::vk::{ComputePipelineCreateInfo, DescriptorSetLayout, Pipeline, PipelineCache, PipelineLayout, PipelineLayoutCreateInfo, PipelineShaderStageCreateInfo, ShaderStageFlags};
use log::error;

use crate::application::{core::error::ErrorCode, scene::scene::Scene, vulkan::{descriptors_helper::allocator::DescriptorAllocator, types::VulkanContext}};
...
pub trait ComputePipeline {
    ...
    fn init_descriptors(&mut self, vulkan_context: &VulkanContext, scene: &Scene) -> Result<(), ErrorCode>;
    ...
    fn init_pipeline_layout(&mut self, vulkan_context: &VulkanContext) -> Result<(), ErrorCode> {
        let set_layouts = self.get_attributes()?.descriptors.iter().map(
            |descriptor|{
        ...
        let pipeline_layout = match unsafe {device.create_pipeline_layout(&create_info, allocation_callback)}{
            Ok(layout) => layout,
            Err(err) => {
                error!("Failed to create a pipeline layout for the test pipeline: {:?}", err);
    ...
    fn init_pipeline(&mut self, shader_name: &str, entry_point: &str, vulkan_context: &VulkanContext) -> Result<(), ErrorCode>{
        let device = vulkan_context.get_device()?;
        let shader_module = match Shader::load_shader_module(
            Shader::get_compiled_shader_path(shader_name),
            &device,
        ){
            Ok(module) => module,
            Err(err) => {
                error!("Failed to load the vulkan shader module `{:?}': {:?}", shader_name, err);
                return Err(ErrorCode::VulkanFailure);
            }
        };
        ...
        let allocation_callback = vulkan_context.get_allocation_callback()?;
        let pipeline = match unsafe{device.create_compute_pipelines(
            PipelineCache::null(), 
            ...
            Err(err) => {
                error!("Failed to create a compute pipeline for the shader `{:?}': {:?}", shader_name, err);
                return Err(ErrorCode::VulkanFailure);
            }
        };
    ...
    fn init(&mut self, shader_name: &str, entry_point: &str, vulkan_context: &VulkanContext, scene: &Scene) -> Result<(), ErrorCode> {
        if let Err(err) = self.init_descriptors(vulkan_context, scene){
            error!("Failed to initialize the vulkan descriptors in a compute pipeline: {:?}", err);
            return Err(ErrorCode::InitializationFailure);
        }
        if let Err(err) = self.init_pipeline_layout(vulkan_context){
            error!("Failed to initialize the vulkan pipeline layout in a compute pipeline: {:?}", err);
            return Err(ErrorCode::InitializationFailure);
        }
        if let Err(err) = self.init_pipeline(shader_name, entry_point, vulkan_context){
            error!("Failed to initialize the vulkan pipeline handler in a compute pipeline: {:?}", err);
            return Err(ErrorCode::InitializationFailure);
        }
    ...
    fn clean_descriptors(&self, vulkan_context: &VulkanContext) -> Result<(), ErrorCode>{
        ...
        for descriptor in &attributes.descriptors {
            unsafe {device.destroy_descriptor_set_layout(descriptor.set_layout, allocation_callback)};
        }
        attributes.descriptor_allocator.destroy_pool(device, allocation_callback)?;
        Ok(())
    }
    ...
    fn clean(&self, vulkan_context: &VulkanContext) -> Result<(), ErrorCode> {
        if let Err(err) = self.clean_descriptors(vulkan_context){
            error!("Failed to clean the vulkan descriptors in a compute pipeline: {:?}", err);
            return Err(ErrorCode::CleaningFailure);
        }
        if let Err(err) = self.clean_pipeline(vulkan_context){
            error!("Failed to clean the vulkan pipeline handler in a compute pipeline: {:?}", err);
            return Err(ErrorCode::CleaningFailure);
        }
        Ok(())
    }
